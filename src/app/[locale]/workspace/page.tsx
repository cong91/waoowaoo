...
import {
	...
	CREATE_WORKSPACE_MODAL_OVERLAY_CLASS,
...
const VISUAL_FIRST_STYLE_PRESETS: Record<
	WorkspaceProjectEntryMode,
	VisualFirstStylePreset[]
> = {
	story: [
		{
...
export default function WorkspacePage() {
	...
	const selectedStarterTemplate =
		useMemo<WorkspaceStarterTemplate | null>(() => {
			if (starterTemplates.length === 0) return null;
			return (
				starterTemplates.find(
					(template) => template.id === formData.starterTemplateId,
				) || starterTemplates[0]
			);
	...
	const selectedStylePreset = useMemo<VisualFirstStylePreset | null>(() => {
		...
		return (
			visualStylePresets.find(
				(preset) => preset.id === formData.stylePresetId,
			) || visualStylePresets[0]
		);
	}, [formData.stylePresetId, visualStylePresets]);

	const selectedCharacterStrategy =
		useMemo<CharacterStrategyOption | null>(() => {
			return (
				VISUAL_FIRST_CHARACTER_STRATEGIES.find(
					(strategy) => strategy.id === formData.characterStrategyId,
				) || VISUAL_FIRST_CHARACTER_STRATEGIES[0]
			);
		}, [formData.characterStrategyId]);

	const selectedEnvironmentPreset =
		useMemo<EnvironmentPresetOption | null>(() => {
			return (
				VISUAL_FIRST_ENVIRONMENT_PRESETS.find(
					(preset) => preset.id === formData.environmentPresetId,
				) || VISUAL_FIRST_ENVIRONMENT_PRESETS[0]
			);
	...
	const comparePresetCandidates = useMemo(() => {
		...
		const current =
			selectedStylePreset &&
			!ordered.some((preset) => preset.id === selectedStylePreset.id)
	...
	const createModalCopy = useMemo(
		() => getCreateWorkspaceModalCopy(formData.entryMode),
		[formData.entryMode],
	);
	...
	const trackWizardStepEvent = useCallback(
		...
		[
			...
			formData.environmentPresetId,
			formData.promptMode,
			formData.referenceBoardSelections,
	...
	const canContinueToSourceStep =
		Boolean(selectedStarterTemplate) &&
		Boolean(selectedStylePreset) &&
		Boolean(selectedCharacterStrategy) &&
		Boolean(selectedEnvironmentPreset);
	const requiresSourceContent = formData.sourceType !== "blank";
	const hasValidSourceContent =
		!requiresSourceContent || formData.sourceContent.trim().length >= 20;
	const canSubmitJourney =
		canContinueToTemplateStep &&
		canContinueToSourceStep &&
	...
	const resetCreateWizard = (
		entryMode: WorkspaceProjectEntryMode = "story",
	) => {
		...
		try {
			...
			if (response.ok) {
				...
				const createdProjectId =
					typeof data?.project?.id === "string" ? data.project.id : "";
				...
				resetCreateWizard(formData.entryMode);
	...
	useEffect(() => {
		...
		formData.environmentPresetId,
		locale,
		recommendedNextMove.description,
		recommendedNextMove.title,
	...
	return (
		<div className="glass-page min-h-screen">
					...
					>
						...
						<div className="relative z-10 flex h-full flex-col justify-between gap-4">
							<div className="flex items-start justify-between gap-3">
								<div>
									<div className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--glass-tone-info-fg)]">
										{t("journeyCardFilmLabel")}
									</div>
									<h3 className="mt-2 text-lg font-bold text-[var(--glass-text-primary)]">
										{t("projectTypeStoryTitle")}
									</h3>
									<p className="mt-2 text-sm text-[var(--glass-text-secondary)] leading-relaxed">
						...
						: projects.map((project) => (
								...
								>
									...
									<div className="p-5 relative z-10">
										...
										<div className="absolute top-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
											...
											>
												{deletingProjectId === project.id ? (
													...
													<AppIcon
														name="trash"
														className="w-4 h-4 text-[var(--glass-tone-danger-fg)]"
													/>
												)}
											</button>
										</div>
										...
										<div className="mb-3">
											<span
												className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold tracking-[0.08em] uppercase ${
													project.onboardingContext?.journeyType ===
													"manga_webtoon"
										...
										{(project.description ||
											project.stats?.firstEpisodePreview) && (
											<div className="flex items-start gap-2 mb-4">
												<AppIcon
													name="fileText"
													className="w-4 h-4 text-[var(--glass-text-tertiary)] mt-0.5 flex-shrink-0"
												/>
												<div className="min-w-0">
													<div className="text-[11px] uppercase tracking-[0.08em] text-[var(--glass-text-tertiary)] mb-1">
														{project.description
															? t("projectCardSourceDescription")
															: t("projectCardSourceStory")}
													</div>
												...
												<div className="flex items-center gap-3 text-sm font-semibold bg-gradient-to-r from-blue-500 to-cyan-500 bg-clip-text text-transparent">
													{project.stats.episodes > 0 && (
														<span
															...
															title={t("statsEpisodes")}
														>
															<AppIcon
																name="statsEpisodeGradient"
				...
				{!loading && projects.length === 0 && (
					<div className="text-center py-12">
						<div className="w-16 h-16 bg-[var(--glass-bg-muted)] rounded-xl flex items-center justify-center mx-auto mb-4">
							<AppIcon
								name="folderCards"
								className="w-8 h-8 text-[var(--glass-text-tertiary)]"
							/>
						</div>
							...
							.map((page, index, array) => (
								<span key={page} className="flex items-center">
									...
									{index > 0 && array[index - 1] !== page - 1 && (
										<span className="px-2 text-[var(--glass-text-tertiary)]">
											...
										</span>
									)}
									<button
										onClick={() => handlePageChange(page)}
										className={`glass-btn-base px-4 py-2 ${
			...
			{showCreateModal && (
				<div className={CREATE_WORKSPACE_MODAL_OVERLAY_CLASS}>
					<div className={CREATE_WORKSPACE_MODAL_SURFACE_CLASS}>
						<form
							onSubmit={handleCreateProject}
							className="flex max-h-[inherit] flex-col"
						>
							<div className={CREATE_WORKSPACE_MODAL_BODY_CLASS}>
								<div className="space-y-5">
									<div className="space-y-2">
										<div className="flex items-start justify-between gap-3">
											<div>
												<h2 className="text-[1.65rem] font-bold leading-tight text-[var(--glass-text-primary)]">
													{t("createProject")}
												</h2>
												<p className="mt-1 max-w-2xl text-sm leading-6 text-[var(--glass-text-tertiary)]">
													{t(createModalCopy.modalIntroKey)}
												</p>
											</div>
										</div>
									</div>

									<div className={CREATE_WORKSPACE_STEP_GRID_CLASS}>
										<span
											className={getCreateWorkspaceStepBadgeClass(
												createWizardStep === 1,
											)}
										>
											1. {t(createModalCopy.stepOneLabelKey)}
										</span>
										<span
											className={getCreateWorkspaceStepBadgeClass(
												createWizardStep === 2,
										...
										>
											3. {t("wizard.stepSource")}
										</span>
									</div>

									{createWizardStep === 1 && (
										<section className="space-y-4 pb-1">
											<div className="rounded-2xl border border-[var(--glass-border)]/55 bg-[linear-gradient(180deg,rgba(255,255,255,0.82),rgba(249,251,255,0.66))] px-4 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.35)]">
												<div className="mb-3">
													<p className="glass-field-label">
														{t("wizard.projectDetailsLabel")}
													</p>
													<p className="mt-1 max-w-2xl text-xs leading-5 text-[var(--glass-text-tertiary)]">
														{t(createModalCopy.projectDetailsHintKey)}
													</p>
												</div>
									...
									{createWizardStep === 2 && (
										<section className="space-y-4 pb-1">
											<div>
												<span className="glass-field-label block mb-2">
													{t("visualFirst.style.title")}
												</span>
												<div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
													{visualStylePresets.map((preset) => {
														const isActive =
															selectedStylePreset?.id === preset.id;
														return (
															<button
																key={preset.id}
																type="button"
																onClick={() => {
																	const journeyType = mapEntryModeToJourneyType(
																		formData.entryMode,
																	);
																	trackWorkspaceJourneyEvent(
															...
															>
																<div className="text-[10px] uppercase tracking-[0.12em] text-[var(--glass-text-tertiary)]">
																	{t(preset.badgeKey)}
																</div>
																<div className="text-sm font-semibold text-[var(--glass-text-primary)] mt-1">
																	{t(preset.titleKey)}
																</div>
																<div className="text-xs text-[var(--glass-text-secondary)] mt-1 line-clamp-2">
																	{t(preset.descKey)}
																</div>
															</button>
														);
											...
											<div>
												<span className="glass-field-label block mb-2">
													{t("visualFirst.character.title")}
												</span>
												<div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
													{VISUAL_FIRST_CHARACTER_STRATEGIES.map((strategy) => {
														const isActive =
															formData.characterStrategyId === strategy.id;
														return (
															<button
																key={strategy.id}
																type="button"
																onClick={() =>
																	setFormData((prev) => ({
																		...prev,
																		characterStrategyId: strategy.id,
																	}))
																}
																className={`w-full glass-btn-base px-3 py-3 text-left ${isActive ? "glass-btn-primary ring-2 ring-[var(--glass-primary)]/30" : "glass-btn-secondary"}`}
															>
											...
											<div>
												<span className="glass-field-label block mb-2">
													{t("visualFirst.environment.title")}
												</span>
												<div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
													{VISUAL_FIRST_ENVIRONMENT_PRESETS.map((preset) => {
														const isActive =
															formData.environmentPresetId === preset.id;
														return (
															<button
																key={preset.id}
																type="button"
											...
											<div className="rounded-lg border border-[var(--glass-border)]/60 bg-[var(--glass-background-secondary)]/35 p-3">
												<div className="text-xs uppercase tracking-[0.12em] text-[var(--glass-text-tertiary)]">
													{t("visualFirst.recommendation.label")}
												</div>
												<div className="text-sm font-semibold text-[var(--glass-text-primary)] mt-1">
													{recommendedNextMove.title}
												</div>
												...
												<div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
													<div className="rounded-md bg-[var(--glass-bg-muted)]/60 px-3 py-2">
														<div className="text-[10px] uppercase tracking-[0.08em] text-[var(--glass-text-tertiary)]">
															{t(
																"visualFirst.recommendation.currentSelectionLabel",
															)}
														</div>
														<div className="mt-1 text-[var(--glass-text-primary)]">
															{selectedStylePreset
																? t(selectedStylePreset.titleKey)
																: "-"}
														</div>
													</div>
													<div className="rounded-md bg-[var(--glass-bg-muted)]/60 px-3 py-2">
														<div className="text-[10px] uppercase tracking-[0.08em] text-[var(--glass-text-tertiary)]">
															{t("visualFirst.recommendation.whyLabel")}
														</div>
														<div className="mt-1 text-[var(--glass-text-primary)]">
															{recommendedNextMove.description}
														</div>
												...
												<div className="mt-3">
													<div className="text-[10px] uppercase tracking-[0.08em] text-[var(--glass-text-tertiary)] mb-2">
														{t("visualFirst.recommendation.compareLabel")}
													</div>
													<div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
														{comparePresetCandidates.map((preset, index) => {
															const isSelected =
																preset.id === selectedStylePreset?.id;
															...
															return (
																...
																>
																	<div className="flex items-center justify-between gap-2">
																		...
																		{isRecommended && (
																			<span className="text-[10px] uppercase tracking-[0.08em] text-[var(--glass-tone-info-fg)]">
																				{t(
																					"visualFirst.recommendation.recommendedBadge",
																				)}
																			</span>
																		)}
																	</div>
																	<div className="text-xs text-[var(--glass-text-secondary)] mt-1 line-clamp-3">
																		{t(preset.descKey)}
																	</div>
																</button>
															);
												...
												<div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-[var(--glass-text-secondary)]">
													...
													<div className="rounded-md bg-[var(--glass-bg-muted)]/60 px-2 py-2">
														<div className="text-[10px] uppercase tracking-[0.08em] text-[var(--glass-text-tertiary)]">
															{t("visualFirst.summary.character")}
														</div>
														<div className="mt-1 text-[var(--glass-text-primary)]">
															{selectedCharacterStrategy
																? t(selectedCharacterStrategy.titleKey)
																: "-"}
														</div>
													</div>
													<div className="rounded-md bg-[var(--glass-bg-muted)]/60 px-2 py-2">
														<div className="text-[10px] uppercase tracking-[0.08em] text-[var(--glass-text-tertiary)]">
															{t("visualFirst.summary.environment")}
														</div>
														<div className="mt-1 text-[var(--glass-text-primary)]">
															{selectedEnvironmentPreset
																? t(selectedEnvironmentPreset.titleKey)
																: "-"}
														</div>
													</div>
												</div>
									...
									{createWizardStep === 3 && (
										<section className="space-y-4 pb-1">
											<div>
												<span className="glass-field-label block mb-2">
													{t("visualFirst.referenceBoard.title")}
												</span>
												<div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
													{VISUAL_FIRST_REFERENCE_BOARD_OPTIONS.map(
														(option) => {
															const isActive =
																formData.referenceBoardSelections.includes(
																	option.id,
																);
															return (
																<button
																	...
																	onClick={() => {
																		...
																		setFormData((prev) => {
																			const nextSelections =
																				prev.referenceBoardSelections.includes(
																					option.id,
																				)
																					? prev.referenceBoardSelections.filter(
																							(id) => id !== option.id,
																						)
																					: [
																							...prev.referenceBoardSelections,
																							option.id,
																						];
																			return {
																				...prev,
												...
												<div className="mt-2 text-xs text-[var(--glass-text-tertiary)]">
													{t("visualFirst.referenceBoard.selectionCount", {
														count: formData.referenceBoardSelections.length,
													})}
												</div>
											</div>
											...
											<div>
												<span className="glass-field-label block mb-2">
													{t("visualFirst.promptMode.title")}
												</span>
												<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
													{(
														["guided", "advanced"] as OnboardingPromptMode[]
													).map((mode) => (
														<button
															key={mode}
															type="button"
														...
														>
															<div className="text-sm font-semibold text-[var(--glass-text-primary)]">
																{t(`visualFirst.promptMode.${mode}.title`)}
															</div>
															<div className="text-xs text-[var(--glass-text-secondary)] mt-1">
																{t(`visualFirst.promptMode.${mode}.desc`)}
															</div>
														</button>
													))}
												</div>
												...
												<div>
													...
													{!hasValidSourceContent && (
														<p className="mt-2 text-xs text-[var(--glass-tone-warning-fg)]">
															{t("wizard.sourceContentHint")}
														</p>
													)}
												</div>
											)}

											<div className="rounded-2xl border border-[var(--glass-border)]/55 bg-[var(--glass-background-secondary)]/45 px-4 py-3 text-xs leading-5 text-[var(--glass-text-secondary)] shadow-[inset_0_1px_0_rgba(255,255,255,0.28)]">
												{t("wizard.readinessSummary", {
													journey:
														formData.entryMode === "manga"
															? t("projectTypeMangaTitle")
															: t("projectTypeStoryTitle"),
													template: selectedStylePreset
														? t(selectedStylePreset.titleKey)
														: "-",
													sourceType: t(
														`wizard.sourceType.${formData.sourceType}`,
													),
												})}
											</div>
										</section>
							...
							<div className={CREATE_WORKSPACE_MODAL_FOOTER_CLASS}>
								...
								<div className="flex w-full justify-end md:w-auto">
									{createWizardStep < 3 ? (
										<button
											...
											onClick={() => {
												if (
													...
													return;
												if (createWizardStep === 2 && !canContinueToSourceStep)
													return;
											...
											className="glass-btn-base glass-btn-primary min-w-[200px] px-4 py-2.5 disabled:opacity-50 sm:min-w-[224px]"
											disabled={
												createLoading ||
												(createWizardStep === 1
													? !canContinueToTemplateStep
										...
										>
											{createWizardStep === 1
												? t(createModalCopy.nextToTemplateKey)
												: t("wizard.nextToSource")}
										</button>
									) : (
										<button
											type="submit"
										...
										>
											{createLoading
												? t("creating")
												: t(createModalCopy.startCreatingKey)}
										</button>
									)}
								</div>
							</div>
